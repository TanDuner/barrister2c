...
- (void)layoutSubviews {
	...
	CGFloat maxWidth = bounds.size.width - 2 * margin - 2 * outMargin;
	...
	CGFloat remainingHeight = bounds.size.height - totalSize.height - kPadding - 2 * margin - 2 * outMargin;
	...
	if (square) {
		...
		if (max <= bounds.size.width - 2 * outMargin) {
			totalSize.width = max;
		}
		if (max <= bounds.size.height - 2 * outMargin) {
